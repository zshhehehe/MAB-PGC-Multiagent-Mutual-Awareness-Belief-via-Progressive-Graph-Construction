...
class Agents:
    def __init__(self, args):
        ...
        elif args.alg == 'mab_pgc':
            from policy.mab_pgc import MAB_PGC
            self.policy = CDPG(args)
        elif args.alg == 'coma':
    ...
    def choose_action(self, obs, last_action, agent_num, avail_actions, epsilon, maven_z=None, evaluate=False):
        ...
        if self.args.alg == 'maven':
            ...
            self.policy.eval_hidden[:, agent_num, :],x = self.policy.eval_rnn(inputs,hidden_state)
            mess = self.policy.eval_query( self.policy.eval_hidden[:, agent_num, :].cuda())
            q_value = self.policy.eval_rnn.forward2(mess,self.policy.eval_hidden[:, agent_num, :])
